...
class TestReadOnlySessionInfo:
    ...
    def test_readonly_session_info_has_read_only_marker(
    ...
    def test_readwrite_session_info_has_read_only_false(
        self, postgres_session: SQLAlchemySession
    ) -> None:
    ...
    def test_default_session_has_no_read_only_marker(
        self, postgres_session: SQLAlchemySession
    ) -> None:
...
class TestBeforeCommitEventPrevention:
    ...
    def test_before_commit_event_raises_error(
        self, postgres_session: SQLAlchemySession
    ) -> None:
        ...
        gene = Gene(
            approved_symbol="RO_TEST1", approved_name="Read Only Test"
        )
        ...
        assert "Cannot commit changes in a read-only session" in str(
            exc_info.value
        )

    def test_error_message_suggests_readwrite_session(
        self, postgres_session: SQLAlchemySession
    ) -> None:
...
class TestSessionInfoForAudit:
    ...
    def test_user_info_set_for_audit(
        self, postgres_session: SQLAlchemySession
    ) -> None:
    ...
    def test_readwrite_session_with_user(
        self, postgres_session: SQLAlchemySession
    ) -> None:
    ...
    def test_readwrite_session_without_user_defaults_to_unknown(
        self, postgres_session: SQLAlchemySession
    ) -> None:
        ...
        from genew4_orm.audit import get_entity_info
...
class TestReadOnlySessionBehavior:
    ...
    def test_readonly_session_does_not_create_audit_logs(
        self, postgres_session: SQLAlchemySession
    ) -> None:
        ...
        # Create a gene
        gene = Gene(
            approved_symbol="RO_AUDIT_TEST", approved_name="Read Only Audit Test"
        )
        postgres_session.add(gene)
        postgres_session.commit()
    ...
    def test_readwrite_session_creates_audit_logs(
        self, postgres_session: SQLAlchemySession
    ) -> None:
        ...
        gene = Gene(
            approved_symbol="RW_AUDIT_TEST", approved_name="Read Write Audit Test"
        )
        postgres_session.add(gene)
...
class TestSessionOperations:
    ...
    def test_select_operation(
        ...
        stmt = select(Gene).where(
            Gene.approved_symbol.like(f"SELECT{unique_suffix}_TEST_%")
        )
...
class TestTransactionBehavior:
    ...
    def test_transaction_isolation(
        ...
        stmt = select(Gene).where(
            Gene.approved_symbol.like(f"ISO{unique_suffix}_TEST_%")
        )
        results = postgres_session.execute(stmt).scalars().all()

        assert len(results) == 2

    def test_multiple_commits_in_one_session(
        self, postgres_session: SQLAlchemySession
    ) -> None:
        ...
        # Both should exist
        stmt = select(Gene).where(
            Gene.approved_symbol.like(f"MULTI{unique_suffix}_COMMIT_%")
        )
        results = postgres_session.execute(stmt).scalars().all()
